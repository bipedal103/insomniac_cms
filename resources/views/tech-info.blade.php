			...
			<div class="tab-pane fade show active" id="btabs-basic" role="tabpanel">
				<div class="table-responsive">
					<table width="100%" class="table table-head-custom">
						<tbody>
							<tr><td>{{ __('tech-info.server-name') }}:</td><td>{{ $_SERVER['SERVER_NAME'] }}</td></tr>
							<tr><td>{{ __('tech-info.ip-and-proto') }}:</td><td>{{ $_SERVER['SERVER_ADDR'] ?? $_SERVER['SERVER_NAME'] }}:{{ $_SERVER['SERVER_PORT'] }} via {{ $_SERVER['SERVER_PROTOCOL'] ?? '-' }}</td></tr>
							<tr><td>{{ __('global.host') }}:</td><td>{{ gethostbyaddr($_SERVER['SERVER_ADDR'] ?? $_SERVER['SERVER_NAME']) }}</td></tr>
							<tr><td>{{ __('tech-info.software') }}:</td><td>{{ $_SERVER['SERVER_SOFTWARE'] }} via {{ PHP_SAPI }}</td></tr>
							...
							<tr><td>{{ __('tech-info.debug-mode') }}:</td><td>@include('layouts.bool_badge', ['value' => config('app.debug'), 'inverted' => true])</td></tr>
					...
					<table width="100%" class="table table-head-custom">
						<tbody>
							@if(!empty($opcache))
								...
								$start_time = $opcache['opcache_statistics']['last_restart_time'] ? formatLocalTimestamp(Carbon\Carbon::createFromTimestamp($opcache['opcache_statistics']['last_restart_time'])) : '-';
