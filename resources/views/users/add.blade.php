...
$active = $allow_push_notifications = true;
$verified = false;
...
$fields = [
	[
		...
		'condition' => isset($user) && $user->canViewRoute('home'),
		'tag' => 'input',
		'group' => [
		...
		'attributes' => [
			...
			'type' => 'url',
			'value' => isset($user) ? $user->getSilentLoginUrl() : '',
			'readonly' => true
		]
	],
	[
...
@section('content')
	@include('layouts.single_header', ['title' => __('users.title-s'), 'icon' => 'fa fa-user-edit', 'actions' => $actions, 'updated_at' => $updated_at])
