...
$title = $slug = $body = $published_at = $tags = '';

if (isset($blog)) {
	$title = $blog->title;
	$slug = $blog->slug;
	$body = $blog->body;
	$published_at = formatLocalTimestamp($blog->published_at, 'Y-m-d H:i');
	$updated_at = $blog->updated_at;

	$tags = $blog->tags()->with('tag')->get()->map(function ($item, $key) {
...
@section('content')
	@include('layouts.single_header', ['title' => __('blogs.title-s'), 'icon' => 'fa fa-edit', 'actions' => $actions, 'updated_at' => $updated_at])
