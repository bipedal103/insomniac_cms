...
$actions = [
	[
		'type' => 'remove',
		'action' => 'sms-messages.remove-multi',
...
$fields_daterange = !isset($start_date) || !isset($end_date) ? [] : [
	[
		...
		'attributes' => [
			'id' => 'daterangepicker',
			'type' => 'text',
			'value' => formatLocalTimestamp($start_date, 'd/m/Y H:i', null, 'Y-m-d H:i') . ' - ' . formatLocalTimestamp($end_date, 'd/m/Y H:i', null, 'Y-m-d H:i'),
...
@section('content')
	@include('layouts.list_header', ['title' => __('sms-messages.title-m'), 'icon' => 'fa fa-comments', 'path' => 'sms-messages.add', 'actions' => $actions, 'search' => $search])
	<div class="card-body">
		<form class="form">
		...
		<table width="100%" class="table table-head-custom js-datatable">
			...
			<tbody>
				@foreach($sms_messages as $row)
					<tr>
						<td><a href="{{ route('sms-messages.show', $row->id) }}">{{ __('global.view') }}</a></td>
						<td>@include('layouts.sms.status_badge', ['status' => $row->status])</td>
