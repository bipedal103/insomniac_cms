...
class MediaStorage
{
	...
	{
		return $file->getExtensionAttribute();
	}
	...
	private function getProtectedMediaRoute(object $file, array $params = []): string
	{
	...
	private function resolveMediaConfig(array $pairs = []): array
	{
		$resolvedPairs = [];
	...
	private function deleteFromRequest(?array $media = []): self
	{
	...
	private function renameAndReorderFromRequest(?array $media = []): self
	{
	...
	private function registerMediaCollections(array $pairs = []): self
	{
	...
	private function saveFromRequest(array $pairs = []): self
	{
		foreach ($pairs as $key => $options) {
