...
trait HasTags
{
	...
	{
		return !$search ? $query : $query->whereHas('tags', function ($query) use ($search) {
	...
	{
		return empty($tags) ? $query : $query->whereHas('tags', function ($query) use ($tags) {
